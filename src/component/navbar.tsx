...
}: Props & { withSheetClose?: boolean }) => {
  const Wrapper = withSheetClose ? SheetClose : React.Fragment;
  const wrapperProps = withSheetClose ? { asChild: true } : {};
    ...
    <div className={`${mobile ? "flex flex-col space-y-4" : "flex space-x-8"}`}>
      {navItems.map((item) => (
        <Wrapper {...wrapperProps} key={item.id}>
          <Button
            variant="ghost"
            className="hover:text-blue-600 justify-start"
            onClick={() => onItemClick(item.id)}
          >
            {item.label}
          </Button>
